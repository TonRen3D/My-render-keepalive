...
本项目包含 `.github/` 和 `.gitignore` 等**隐藏文件/文件夹**（以点开头）。

- ❌ **不要用网页端拖拽上传**——系统文件选择器默认不显示隐藏文件，会漏掉 `.github/workflows/`
- ✅ **必须用 Git 命令行上传**——`git add -A` 会包含所有隐藏文件
...
# 在 GitHub 上创建空仓库后，运行：
bash setup.sh https://github.com/你的用户名/render-keepalive.git
```
...
git branch -M main
git add -A
git commit -m "init: Render Keepalive"
git remote add origin https://github.com/你的用户名/render-keepalive.git
